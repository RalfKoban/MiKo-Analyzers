...
namespace MiKoSolutions.Analyzers.Rules.Maintainability
{
    ...
    public sealed class MiKo_3029_EventRegistrationOrderAnalyzer : MaintainabilityAnalyzer
    {
        ...
        private void AnalyzeMethodDeclaration(SyntaxNodeAnalysisContext context) => AnalyzeAssignmentExpressions(context);
            ...
            if (assignments.Count > 0)
            {
                ...
                if (addAssignmentsCount != 0 && subtractAssignmentsCount != 0)
                {
                    if (addAssignmentsCount == subtractAssignmentsCount && addAssignmentsCount == 1)
                    {
                        ...
                        if (addAssignment.Left.IsKind(SyntaxKind.SimpleMemberAccessExpression) && addAssignment.Right is IdentifierNameSyntax addedHandler && subtractAssignment.Right is IdentifierNameSyntax removedHandler && addedHandler.GetName() != removedHandler.GetName())
                        {
